...
/**
 * GAP (Generic Access Profile) callback function handling various BLE events,
 * such as scan results, connection status, and more. It allows the application
 * to react to changes in the BLE environment, for example, by connecting to a
 * device that has been found during scanning.
 */
static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
...
/**
 * GATT Client callback function to handle events related to GATT client operations,
 * such as service discovery, characteristic read/write, and more. This function is
 * crucial for interacting with remote GATT servers over BLE.
 */
static void esp_gattc_cb(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
...
/**
 * Initializes BLE functionality, including controller, NVS, and Bluedroid stack.
...
/**
 * Task function to generate wheel ticks. This function toggles a GPIO pin to simulate
 * wheel ticks, using precise timing to control the frequency and duration of the ticks.
 * It runs indefinitely as a FreeRTOS task.
 */
...
Main application entry point. Initializes necessary components (BLE, GPIO)
and starts the task for generating wheel ticks. It sets up BLE for scanning
and potentially connecting to a Wahoo speed sensor.
*/
void app_main(void) {
